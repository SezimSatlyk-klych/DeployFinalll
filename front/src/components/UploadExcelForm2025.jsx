...
const FilesTab = React.memo(function FilesTab({ excel2025Files, filesLoading, fetchFiles, setDeleteDialog, setResetDialog, onDelete }) {
    ...
    <Card sx={{ 
      ...
      <CardContent sx={{ p: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <IconButton 
            onClick={() => onDelete(file, 'excel2025')}
            sx={{ 
              color: '#f44336',
              '&:hover': { 
      ...
      {filesLoading ? (
        ...
        <Box sx={{ mb: 4 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            ...
            {excel2025Files.length > 0 && (
              <Button
                startIcon={<WarningIcon />}
                onClick={() => setResetDialog({ open: true, type: 'excel2025' })}
                variant="outlined"
                color="error"
                size="small"
                sx={{ borderRadius: 2, fontWeight: 600 }}
          ...
          {excel2025Files.length === 0 ? (
            <Card sx={{ p: 3, textAlign: 'center', background: '#fafafa' }}>
              <Typography color="text.secondary">
                Нет загруженных файлов CRM 2025
              </Typography>
            </Card>
          ) : (
            excel2025Files.map((file, index) => (
              <FileCard 
                key={index} 
                file={file} 
                type="excel2025" 
...
function UploadExcelForm2025() {
  ...
  const [excel2025Files, setExcel2025Files] = useState([]);
  ...
  const handleResetAll = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setSuccess(`Все данные ${type === 'crm' ? 'CRM' : 'CRM 2025'} успешно сброшены`);
        fetchFiles();
      } else {
...
export default UploadExcelForm2025; 