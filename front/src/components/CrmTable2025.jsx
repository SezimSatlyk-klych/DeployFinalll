...
function CrmTable2025({ onProfile }) {
  ...
  const [draftFilters, setDraftFilters] = useState(() => {
    ...
    const saved = localStorage.getItem('crm2025Filters');
    const savedTimestamp = localStorage.getItem('crm2025FiltersTimestamp');
    
    // Проверяем, не устарели ли фильтры (больше 30 минут)
    ...
    // Если фильтры устарели или их нет, очищаем localStorage
    localStorage.removeItem('crm2025Filters');
    localStorage.removeItem('crm2025FiltersTimestamp');
  ...
  const [filters, setFilters] = useState(() => {
    ...
    // Проверяем актуальность фильтров
    if (saved && savedTimestamp) {
      const now = Date.now();
      const savedTime = parseInt(savedTimestamp);
  ...
  const handleEditClick = (row) => {
    setEditRow(row);
    // Преобразуем язык в массив, если он не является массивом
    const editDataWithArrayLanguage = { ...row };
    if (editDataWithArrayLanguage.язык && !Array.isArray(editDataWithArrayLanguage.язык)) {
      editDataWithArrayLanguage.язык = [editDataWithArrayLanguage.язык];
    } else if (!editDataWithArrayLanguage.язык) {
      editDataWithArrayLanguage.язык = [];
    }
    setEditData(editDataWithArrayLanguage);
    setEditSuccess('');
    setEditError('');
  };
  ...
  // Проверяем фильтры при загрузке компонента
  React.useEffect(() => {
  ...
  return (
    <Box sx={{
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 4, justifyContent: 'space-between', flexWrap: 'wrap', gap: 2, background: '#f4f8ff', borderRadius: 3, p: 2, boxShadow: '0 2px 8px 0 rgba(66,165,245,0.07)' }}>
        <Typography variant="h5" fontWeight={800} sx={{ color: '#2563eb', letterSpacing: 0.5 }}>
          {Object.values(filters).some(v => v && (Array.isArray(v) ? v.length > 0 : true)) ? 
            `Отфильтрованные записи (${data.length})` : 'Все записи 2025'}
        </Typography>
        <TextField
          size="small"
          placeholder={Object.values(filters).some(v => v && (Array.isArray(v) ? v.length > 0 : true)) ? 
            "Поиск в отфильтрованных данных..." : "Поиск по таблице..."}
          value={search}
          ...
          <Box sx={{
            ...
            {columns.map(col => (
              <Box key={col} sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                ...
                {col === 'язык' ? (
                  <FormControl size="small" fullWidth sx={{ background: '#f4f8ff', borderRadius: 2 }}>
                    <Select
                      multiple
                      name={col}
                      value={Array.isArray(editData[col]) ? editData[col] : (editData[col] ? [editData[col]] : [])}
                      onChange={handleEditChange}
                      renderValue={(selected) => selected.join(', ')}
                      sx={{ background: '#f4f8ff', borderRadius: 2 }}
                    >
                      {editLanguages.filter(l => l !== '').map(l => (
                        <MenuItem key={l} value={l}>
                          <Checkbox checked={Array.isArray(editData[col]) ? editData[col].indexOf(l) > -1 : false} />
                          {l}
                        </MenuItem>
                      ))}
                    </Select>
